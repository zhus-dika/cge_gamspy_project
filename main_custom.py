...
def main():
    ...
    i = Set(
        ...
        name="i",
        domain=u,
        records=['Primary and Light Industry', 'Heavy Industry and Manufacturing', 'Utilities and Transport', 'Trade and Finance', 'Social and Professional Services'],
        description="goods",
    )
    h = Set(
    ...
    t = Set(
        m, name="t", records=[str(i) for i in range(11)], description="time"
    )
    ...
    Tc0 = Parameter(m, name="Tc0", domain=[i, t], description="consumption tax")
    Te0 = Parameter(m, name="Te0", domain=[i, t], description="export tax")
    ...
    Tc00 = Parameter(m, name="Tc00", domain=i, description="consumption tax")
    Te00 = Parameter(m, name="Te00", domain=i, description="export tax")
    tauc00 = Parameter(m, name="tauc00", domain=i, description="consumption tax rate")
    taue00 = Parameter(m, name="taue00", domain=i, description="export tax rate")

    tauc = Parameter(m, name="tauc", domain=i, description="consumption tax rate")
    taue = Parameter(m, name="taue", domain=i, description="export tax rate")
    tauc[i] = tauc00[i]
    taue[i] = taue00[i]
    ...
    Tc1 = Parameter(m, name="Tc1", domain=[i, t], description="consumption tax")
    Te1 = Parameter(m, name="Te1", domain=[i, t], description="export tax")
    ...
    tauc00[i] = Tc00[i] / Xp00[i]
    taue00[i] = Te00[i] / E00[i]
    F00[h, j] = SAM[h, j]
    Y00[j] = Sum(h, F00[h, j])
    X00[i, j] = SAM[i, j]
    ...
    Tc0[i, t] = Tc00[i] * (1 + pop) ** (Ord(t) - 1)
    Te0[i, t] = Te00[i] * (1 + pop) ** (Ord(t) - 1)
    pf0[h, j, t] = 1
    py0[j, t] = 1
    pz0[j, t] = 1
    pq0[i, t] = 1
    ...
    Tc = Variable(m, name="Tc", type="free", domain=i, description="consumption tax")
    Te = Variable(m, name="Te", type="free", domain=i, description="export tax")
# Equations
    eqpy = Equation(
    ...
    eqTd[...] = Td == Sum(i, pq[i] * Xg[i]) - Sum(i, Tm[i] + Tz[i] + Tc[i] + Te[i])
    ...
    # [international trade] --
    eqpe[i] = pe[i] == epsilon * pWe[i] + taue[i]
    ...
    for iteration, _ in t.records.itertuples(index=False):
        ...
        Tc1[i, iteration] = Tc.l[i]
        Te1[i, iteration] = Te.l[i]
    ...
    dTc = Parameter(
        m, name="dTc", domain=[i, t], description="change of consumption tax [%]"
    )
    dTe = Parameter(
        m, name="dTe", domain=[i, t], description="change of export tax [%]"
    )
    ...
    gTc1 = Parameter(
        ...
        name="gTc1",
        domain=[j, t],
        description="growth of consumption tax              [%]",
    )
    gTe1 = Parameter(
        m,
        name="gTe1",
        domain=[i, t],
        description="growth of export tariff                [%]",
    )
    gFF1 = Parameter(
        m,
        name="gFF1",
    ...
    dTc[i, t].where[Tc0[i, t]] = (Tc1[i, t] / Tc0[i, t] - 1) * 100
    dTe[i, t].where[Te0[i, t]] = (Te1[i, t] / Te0[i, t] - 1) * 100
    ...
    gTc1[i, t + 1].where[Tc1[i, t]] = (Tc1[i, t + 1] / Tc1[i, t] - 1) * 100
    gTe1[i, t + 1].where[Te1[i, t]] = (Te1[i, t + 1] / Te1[i, t] - 1) * 100
